...
function carregarImagem(id) {
    const img = document.getElementById('imgDoce');
    if (!id) {
...
function previewImagem() {
    ...
    if (inputFiles.length > 0) {
        ...
        document.getElementById('imgDoce').src = url;
...
async function procurePorChavePrimaria(chave) {
    try {
        const resposta = await fetch(`${URL_API}/doce/${chave}`);
        const data = await resposta.json();
        return data.sucesso ? data.doce : null;
    } catch (erro) {
        return null;
    }
}
...
async function procure() {
    const id_doce = document.getElementById("id_doce").value;
    if (isNaN(id_doce) || !Number.isInteger(Number(id_doce)) || id_doce === "") {
        mostrarAviso("Precisa ser um número inteiro");
        return;
    }

    doce = await procurePorChavePrimaria(id_doce);
    oQueEstaFazendo = ''; // Reseta o estado
    
    if (doce) {
        mostrarDadosdoce(doce);
        carregarImagem(id_doce);
...
async function salvar() {
    let id_doce = document.getElementById("id_doce").value;
    const nome = document.getElementById("nome").value;
    const tamanho = document.getElementById("tamanho").value;
    const peso = document.getElementById("peso").value;

    const dadosdoce = { id_doce, nome, tamanho, peso};

    try {
        if (oQueEstaFazendo === 'inserindo') {
            await fetch(`${URL_API}/doce`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(dadosdoce) });
            await uploadImagemParaServidor(id_doce); // Salva a imagem após o texto
            mostrarAviso("Inserido no Banco de Dados com sucesso!");
        } else if (oQueEstaFazendo === 'alterando') {
            await fetch(`${URL_API}/doce/${id_doce}`, { method: 'PUT', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(dadosdoce) });
            await uploadImagemParaServidor(id_doce); // Atualiza a imagem após o texto
            mostrarAviso("Alterado no Banco de Dados com sucesso!");
        } else if (oQueEstaFazendo === 'excluindo') {
            await fetch(`${URL_API}/doce/${id_doce}`, { method: 'DELETE' });
            carregarImagem(null);
        ...
        limparAtributos();
        document.getElementById("id_doce").value = "";
        listar();
    } catch (erro) {
...
async function listar() {
    try {
        const resposta = await fetch(`${URL_API}/doces`);
        const data = await resposta.json();
        if (data.sucesso) {
            let texto = "";
            for (let linha of data.doces) {
                texto += `${linha.id_doce} - ${linha.nome} - ${linha.tamanho} - ${linha.peso} <br>`;
            }
            document.getElementById("outputSaida").innerHTML = texto || "Nenhum doce cadastrado.";
        }
    } catch (erro) {
...
function mostrarDadosdoce(f) {
    document.getElementById("id_doce").value = f.id_doce;
    document.getElementById("nome").value = f.nome;
    document.getElementById("tamanho").value = f.tamanho;
...
function bloquearAtributos(soLeitura) {
    document.getElementById("id_doce").readOnly = !soLeitura;
    document.getElementById("nome").readOnly = soLeitura;
    document.getElementById("tamanho").readOnly = soLeitura;
